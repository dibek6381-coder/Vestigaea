...
class VisionSystem(System):
    ...
    def get_stimuli(self, x: float, y: float, direction: float, 
                   range_px: float, fov_deg: float) -> List[Stimulus]:
        """
        ...
        for entity_id in entities:
            ...
            # Skip self
            if transform["x"] == x and transform["y"] == y:
                continue
            ...
            if angle_diff <= half_fov:
                ...
                if self.world.get_component(entity_id, "predator") is not None:
                    stim_type = "predator"
                elif self.world.get_component(entity_id, "shelter") is not None:
                    stim_type = "shelter"
